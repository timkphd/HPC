...
??? note "Is NLR HPC related to the Information Technology Services Desk?"
    ...
    digital device you are issued by NLR. HPC Operations will assist with issues regarding
    HPC systems.  **Note that your NLR HPC account is a different account from your ITS credentials**
...
??? note "What are project allocations?"
    ...
    more about [requesting an allocation](https://www.nlr.gov/hpc/resource-allocation-requests.html).

??? note "How can I access NLR HPC systems?"

    Begin by [requesting an NLR HPC account](https://www.nlr.gov/hpc/user-accounts.html). 
    Then, consult our guide on [how to connect to  NLR HPC systems](https://www.nlr.gov/hpc/system-connection.html).
...
??? note "What are "login" and "compute" nodes?"
    ...
    inappropriately (see 'What is proper NLR HPC login node etiquette' for more detail).
...
??? note "What is proper NLR HPC login node etiquette?"
    ...
    admonished accordingly. For more information please see our [policy](https://www.nlr.gov/hpc/inappropriate-use-policy.html) on what 
    constitutes inappropriate use.

??? note "What is "system time?""

    System time is a regularly occurring interval of time during which NLR HPC systems
    ...
    You can check the [system status page](https://www.nlr.gov/hpc/system-status.html) if you are ever 
    unsure if an NLR HPC system is currently down for system time.
...
??? note "Why aren't my jobs running?"
    ...
    Good question! There may be hundreds of reasons why. Please [contact HPC support](https://www.nlr.gov/hpc/contact-us.html)
