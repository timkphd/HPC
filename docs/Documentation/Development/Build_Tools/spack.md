...
The edited 3 files `compilers.yaml`, `packages.yaml` and `modules.yaml`; meant to be used on Kestrel; are hosted in the following locations:
For CPU nodes installs ( `arch=linux-rhel8-sapphirerapids` ): `/nopt/nrel/apps/cpu_stack/software/spack_files`
For GPU nodes installs ( `arch=linux-rhel8-zen4` ): `/nopt/nrel/apps/gpu_stack/software/spack_files` 
